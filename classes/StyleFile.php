...
class StyleFile {
	...
	function viewMyProducts( $userId, $index, $limit ) {
        ...
        if($result) {
            $query = "select productId, primaryPhoto, isApproved, isPurchased from tbl_products p, tbl_users u, tbl_brands b WHERE p.userId = u.userId and p.userId = $userId and p.brandId = b.brandId and u.isActive = 1 order by productId desc LIMIT $index, $limit";
            $result = mysql_query($query);
	...
	function profileInfo( $arrmixData ) {
		...
		$query = "select userId, username, isFeatured, profilePicURL, city, bio, (select count(ProductId) from tbl_products where userId = $intUserId ) as listed, sold, followers, following from tbl_users where userId='". $intUserId ."'";
		$result = mysql_query($query);
	...
	function saveUserAddresses( $arrData ) {
        ...
        if( false == array_key_exists( 'redirect', $arrData ) || false == $arrData['redirect'] ) {
        	...
        	if($result) {
        		$this -> setMessage("Address details have been updated successfully");
        	}
        	else {
        		$this -> setMessage("Failed to update address");
        	}
        
        	header("Location:../sf_admin_control-panel/editUserDetails.php?userId=" . $arrData['userId'] );
        	exit;
        }
	}
    ...
    function deleteUserAddress( $arrData ) {
        ...
        if( false == array_key_exists( 'redirect', $arrData ) || false == $arrData['redirect'] ) {
        	...
        	if($result) {
        		$this -> setMessage("Address has been resetted successfully");
        	}
        	else {
        		$this -> setMessage("Failed to reset address");
        	}
    ...
    function updateProfile( $arrUser, $profilePicURL ) {
        ...
        if( false == array_key_exists( 'redirect', $arrUser ) || false == $arrUser['redirect'] ) {
            ...
            if($result) {
                $this -> setMessage("Personal details have been updated successfully");
            }
            else {
                $this -> setMessage("Failed to update personal details");
            }
           
            header("Location:../sf_admin_control-panel/editUserDetails.php?userId=" . $arrUser['userId'] );
            exit;
         }
    }
    ...
    function updateKYCDetails( $arrRequestData ) {
        ...
        if( false == array_key_exists( 'redirect', $arrRequestData ) || false == $arrRequestData['redirect'] ) {
        	...
        	if($result) {
        		$this -> setMessage("Bank Details ha been updated successfully");
        	}
        	else {
        		$this -> setMessage("Failed to update bank details");
        	}
        	 
        	header("Location:../sf_admin_control-panel/editUserDetails.php?userId=" . $arrRequestData['userId'] );
        	exit;
        }
