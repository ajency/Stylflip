...
class Product {
	...
	function createProduct($arrProduct) {
		...
		if($successProductImage1 && $successProductImage2 && $successProductImage3 && $successProductImage4 && $successProductImage5) {
			...
			$query = "insert into tbl_products (userId, productTitle, `condition`, productDescription, brandId, categoryId, subcategoryId, primaryPhoto, originalPrice, sellingPrice, discountPrice, discountPercentage, likes, size, sizeChart, isToBeDonated, createdOn $strColumns) values ('".$arrProduct['userId']."', '".$arrProduct['productTitle']."', '".$arrProduct['condition']."', '".$arrProduct['productDescription']."', '".$arrProduct['brandId']."', '".$arrProduct['categoryId']."', '".$arrProduct['subcategoryId']."', '" . $productImage1 . "', '" .$arrProduct['originalPrice'] . "', '" .$arrProduct['sellingPrice'] . "', '" .$arrProduct['discountPrice'] . "', '" .$arrProduct['discountPercentage'] . "', '0', '".$arrProduct['size']."', '".$arrProduct['sizeChart']."', '".$arrProduct['toBeDonated']."', '" . date("Y-m-d H:i:s") . "' $strColumnValues )";
	...
	function listing( $userId, $index, $limit, $searchText, $strSortBy, $brands, $categories, $subcategories, $sizes, $conditions, $priceRange ) {
		...
		if( true == isset( $strSortBy ) && false == empty( $strSortBy ) ) {
			...
			$strOrderBy = ' order by p.productId';
		}
	...
	function like( $userId, $productId ) {
		...
		if($result) {
			$query = "update tbl_products set likes = likes + 1 WHERE productId = $productId AND userId = $userId" ;
			$result = mysql_query($query);
	...
	function dislike( $userId, $productId ) {
		...
		if($result) {
			...
			$query = "update tbl_products set likes = likes - 1 WHERE productId = $productId AND userId = $userId AND likes > 0" ;
			$result = mysql_query($query);
	...
	function getSubcategories( $intCategoryId ) {
		$sql = "select * from tbl_categories WHERE parentCategoryId = $intCategoryId";
		echo json_encode($this -> createResponse($sql));
	}
	...
	function approveProduct( $arr ) {
		...
		if($result) {
            ...
            $query = "insert into tbl_stylefeed (title, lat, lon, city, userId, photo, likes, createdOn) values ('".$feedTitle."', '0', '0', '', '".$userId."', '" . $feedImage . "', '0', '" . date("Y-m-d H:i:s") . "' )";
            $result = mysql_query($query);
    ...
    function updateOrderStatus( $data ) {
        ...
        if($result) {
            $query = "select buyerId, sellerId, orderId from tbl_orders where orderId = '".$data['orderId']."'";
            $result = mysql_query($query);
            ...
            require_once("../classes/PushNotification.php");
            $pushNotification = new PushNotification();
            $pushNotification -> prepareAndSendNotification(0, $buyerId, 'social', 'orderStatusChanged', $data['orderId'], $data['status']);
			$pushNotification -> prepareAndSendNotification(0, $sellerId, 'social', 'orderStatusChanged', $data['orderId'], $data['status']);
