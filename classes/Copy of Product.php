...
class Product {
	...
	function updateProductDetailsCopy( $arrProduct ) {
        require_once("../classes/FileUploader.php");
        $fileUploader = new FileUploader();
		...
		if(isset($_REQUEST['productImage1'])) {
			$productImage1 = str_replace('http://stylflip.com/stylflip/', '', $_REQUEST['productImage1']);
			$successProductImage1 = true;
		}
        ...
        if(isset($_REQUEST['productImage2'])) {
			$productImage2 = str_replace('http://stylflip.com/stylflip/', '', $_REQUEST['productImage2']);
			$successProductImage2 = true;
		}
        ...
        if(isset($_REQUEST['productImage3'])) {
			$productImage3 = str_replace('http://stylflip.com/stylflip/', '', $_REQUEST['productImage3']);
			$successProductImage3 = true;
		}
        ...
        if(isset($_REQUEST['productImage4'])) {
			$productImage4 = str_replace('http://stylflip.com/stylflip/', '', $_REQUEST['productImage4']);
			$successProductImage4 = true;
		}
        ...
        if(isset($_REQUEST['productImage5'])) {
			$productImage5 = str_replace('http://stylflip.com/stylflip/', '', $_REQUEST['productImage5']);
			$successProductImage5 = true;
		}
        ...
        if(!$successProductImage1 || !$successProductImage2 || !$successProductImage3 || !$successProductImage4 || !$successProductImage5) {
            ...
            if($result) {
				...
				echo 'productImage2 => ' . $_REQUEST['productImage2'] . ' | ' . (isset($_FILES['productImage2']) ? 'set' : 'not set') . ' \n ';
				echo 'productImage3 => ' . $_REQUEST['productImage3'] . ' | ' . (isset($_FILES['productImage3']) ? 'set' : 'not set') . ' \n ';
				echo 'productImage4 => ' . $_REQUEST['productImage4'] . ' | ' . (isset($_FILES['productImage4']) ? 'set' : 'not set') . ' \n ';
				echo 'productImage5 => ' . $_REQUEST['productImage5'] . ' | ' . (isset($_FILES['productImage5']) ? 'set' : 'not set');
    ...
    function purchaseProduct( $data ) {
        ...
        if($result) {
            if($data['type'] == 'success') {
                ...
                $query = "select userId, productId, productTitle, primaryPhoto from tbl_products where productId = '" . $data['productId'] . "'";
                $result = mysql_query($query);
                ...
                $query = "select userId, firstName, lastName, email from tbl_users where userId = $sellerId";
                $result = mysql_query($query);
    ...
    function updateOrderStatus( $data ) {
        ...
        if($result) {
            $query = "select p.productId, p.productTitle, o.productId, o.buyerId, o.sellerId, o.orderId from tbl_orders o, tbl_products p where p.productId = o.productId and o.orderId = '".$data['orderId']."'";
            $result = mysql_query($query);
	...
	function updateOrderStatusCopy( $data ) {
        ...
        if(TRUE) {
			...
			$sellerEmail = 'sushant.ahirrao@icloud.com';
		 	...
		 	switch($data['status']) {
				...
				case 'Complete':
					...
					if($sellerEmail != '') {
						...
						$body = $mailer -> getCompiledTemplate('../emailTemplates/orderCompleteToSeller.html', $variables);
