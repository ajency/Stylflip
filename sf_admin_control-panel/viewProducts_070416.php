...
<tr>
	  <td colspan="16" align="right"><form method="post" action="viewProducts.php"><input type="text" placeholder="Search product" name="search_text" value="<?php if(isset($_REQUEST['search_text']))echo $_REQUEST['search_text']; ?>"><input type="submit" value="Search"> | <a href="viewProducts.php">View All</a><?php if($isSuperAdmin) { ?> | <a href="downloadReport.php?exportType=productsData">Export</a><?php } ?></form></td>
</tr>	
...
?>
<tr>
    <td colspan="16"><div align="right"><strong>Page  - </strong> <?php echo $pager->renderNav('<span>', '</span>'); ?>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</div></td>  
</tr>	
  <tr>
    <td colspan="16"></td>  
  </tr>
...
<tr>
	...
	<td>
		<ul style="list-style: none; padding-left: 0px;">
			...
			<?php if( '1' <> $row['isApproved'] || true == is_null($row['isApproved']) ) { ?><li><a style="cursor: pointer;" onclick="approveProduct('<?php echo $baseURL; ?>api/product.php?action=approveProduct&productId=<?php echo $productId ?><?php if(isset($_REQUEST['page'])) { echo "&page=" . $_REQUEST['page']; }  ?>')">Approve</a></li><?php } ?>
			<?php if( '1' == $row['isApproved'] ) { ?><li><a style="cursor: pointer;" onclick="rejectProduct('<?php echo $baseURL; ?>api/product.php?action=rejectProduct&productId=<?php echo $productId ?><?php if(isset($_REQUEST['page'])) { echo "&page=" . $_REQUEST['page']; }  ?>')">Reject</a></li><?php } ?>
			<li><a href="editProductDetails.php?productId=<?php echo $productId; ?><?php if(isset($_REQUEST['page'])) { echo "&page=" . $_REQUEST['page']; }  ?>">Edit</a></li>
			<li><a style="cursor: pointer;" onclick="deleteProduct('<?php echo $baseURL; ?>api/product.php?action=deleteProduct&isAdmin=true&productId=<?php echo $productId ?><?php if(isset($_REQUEST['page'])) { echo "&page=" . $_REQUEST['page']; }  ?>')">Delete</a></li>
		</ul>
	</td>
<?php	
		$sr=$sr+1;
