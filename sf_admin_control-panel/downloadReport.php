<?php
	...
	if($_REQUEST['exportType'] == 'ordersData') {
		...
		$query = "select o.*, u.userId as sellerId, u.firstName as sellerFirstName, u.email as sellerEmail, u.lastName as sellerLastName, p.productTitle, p.isToBeDonated, p.pickupFrom, p.sellingPrice from tbl_orders o, tbl_products p, tbl_users u where o.productId = p.productId and p.userId = u.userId order by o.orderId desc";
		$result = mysql_query($query);
		
		
		$data = array();
		array_push($data, array('Sr no', 'Order no', 'Status', 'Order Date', 'Seller name', 'Buyer name', 'SKU', 'Product title', 'Payment Mode', 'Transaction ID', 'PayU ID', 'Final amount paid', 'Asking price', 'SF comission', 'S & H', 'Display price','COD charges', 'Coupon used', 'Buyer address', 'Buyer city', 'Buyer state', 'Buyer post code', 'Buyer contact no', 'Buyer Email', 'Seller pickup address', 'Seller pickup city', 'Seller pickup state', 'Seller pickup post code', 'Seller contact no', 'Seller Email', 'Reverse AWB (Pick up from seller)', 'Reverse Courier Name', 'Forward AWB (to buyer)', 'Forward Courier Name','QC Date','Delivery Date','Payment Reference Nodal','Payment Reference Current','Payment Date'));
		array_push($data, array('', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', ''));
		...
		while($row = mysql_fetch_assoc($result)) {
		 ...
		 if($row['qcdate']=='0000-00-00'){ $qcdt=''; }else { $qcdt=EntryDate($row['qcdate']);} 
	if($row['delivery_date']=='0000-00-00'){ $dldt=''; }else { $dldt=EntryDate($row['delivery_date']);} 
	if($row['payment_date']=='0000-00-00'){ $paydt=''; }else { $paydt=EntryDate($row['payment_date']);} 
 
		$couponDiscount=$row['paymentMode'] == 'Cash on delivery' ? 'Rs. 85' : 'Rs. 0';
			...
			$query1 = "select u.email as BuyerEmail from tbl_orders o, tbl_products p, tbl_users u where o.productId = p.productId and o.buyerId = u.userId order by o.orderId desc";
			
			$BuyerResult = mysql_query($query1);
			$BuyerEmail = mysql_fetch_assoc($BuyerResult);
			...
			array_push($data, array(
				...
				$qcdt,
				$dldt,
				$row['payment_reference_nodal'],
				$row['payment_reference_current'],
				$paydt
