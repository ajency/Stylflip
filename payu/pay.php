...
if(isset($_REQUEST['cashOnDelivery']) || $_REQUEST['amount'] < 50) {
	if(!$_REQUEST['amount']) {
		$_REQUEST['amount'] = 85;
	}
	
	$_REQUEST['mode'] = 'Cash on delivery';
	payment_success();
}
else {
	/* Payments made easy. */
    ...
    function createOrder($data) {
       ...
       // print_r($data['sellerEmail']); die;
        $query = "insert into tbl_orders (buyerId, sellerId, productId, amount, originalPrice, discount, addressLine1, addressLine2, city, landmark, phoneNumber, postCode, state, firstName, lastName, status, orderDate, transactionId, payUTransactionId, type, payUStatus, paymentMode, cardCategory, amountDebitedByPayU, pgType, bankRefNumber, bankCode, nameOnCard, cardNumber, issuingBank, cardType, errorCode, errorMessage, couponCodeUsed) values ('".$data['buyerId']."', '".$data['sellerId']."', '".$data['productId']."', '".$data['amount']."', '".$data['originalPrice']."', '".$data['discount']."', '".$data['addressLine1']."', '".$data['addressLine2']."', '".$data['city']."', '".$data['landmark']."', '".$data['phoneNumber']."', '".$data['postCode']."', '".$data['state']."', '".$data['firstName']."', '".$data['lastName']."', 'Processing', '".date("d/m/Y H:i:s")."', '".$data['transactionId']."', '".$data['payUTransactionId']."', '".$data['type']."', '".$data['payUStatus']."', '".$data['paymentMode']."', '".$data['cardCategory']."', '".$data['amountDebitedByPayU']."', '".$data['pgType']."', '".$data['bankRefNumber']."', '".$data['bankCode']."', '".$data['nameOnCard']."', '".$data['cardNumber']."', '".$data['issuingBank']."', '".$data['cardType']."', '".$data['errorCode']."', '".$data['errorMessage']."', '".$data['couponCode']."')";
        $result = mysql_query($query);
        ...
        if($result) {
            if($data['type'] == 'success') {
                ...
                if($data['sellerEmail'] != '') {
				//echo '111'; die;
					$query = "select pickupFrom from tbl_products where productId='".$data['productId']."'";
					$result = mysql_query($query);
					$productDetails = mysql_fetch_assoc($result);
					
					$query = "select * from tbl_user_addresses where userId='". $data['sellerId'] ."' and addressTitle='".$productDetails['pickupFrom']."'";
                ...
                while($row1 = mysql_fetch_assoc($result)) {
                    ...
                    $variables = array(
                                       ...
                                       'sellerPhoneNumber' => $addressesRow['phoneNumber'],
