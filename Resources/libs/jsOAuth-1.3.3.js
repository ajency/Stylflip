                ...
                xhr.onreadystatechange = function () {
                    if (this.readyState === 4) {
                        ...
                        if (!!this.getAllResponseHeaders) {
                            responseHeadersString = this.getAllResponseHeaders();
                            while((match = regex.exec(responseHeadersString))) {
                                responseHeaders[match[1]] = match[2];
                            }
                        } else if(!!this.getResponseHeaders) {
                            responseHeadersString = this.getResponseHeaders();
                        ...
                        var responseObject = {text: this.responseText, xml: (includeXML ? this.responseXML : ''), requestHeaders: requestHeaders, responseHeaders: responseHeaders};

                        // we are powerless against 3xx redirects
                        if((this.status >= 200 && this.status <= 226) || this.status == 304 || this.status === 0) {
